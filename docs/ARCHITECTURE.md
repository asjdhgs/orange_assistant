...
项目采用 Rust/Axum 构建 Web 服务。页面、静态资源和 API 由同一个进程托管，服务统一处理页面渲染、静态资源、认证、MBTI、数据库访问、推荐算法、知识图谱、风险分析和外部模型代理。
...
Axum 路由层 routes.rs + frontend.rs
        ...
        ├─ recommendation.rs 院校筛选、加权排序、冲稳保分档
        ├─ portfolio.rs      组合风险、敏感性、蒙特卡洛模拟
        ├─ knowledge.rs      知识图谱解析、匹配、BFS 遍历
        └─ llm.rs            DeepSeek 调用与无密钥降级
                │
                ▼
        MySQL / 内置演示数据
```
...
- 推荐 SQL 只执行固定的只读查询。
- `.env` 被 Git 忽略，API 密钥不进入源码。
- 大模型未配置或调用失败时自动使用本地算法结果，核心功能仍可运行。
